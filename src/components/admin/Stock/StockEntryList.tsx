...
export default function StockEntryList() {
  ...
  const { data, isLoading, isSuccess, refetch } = useQuery<{
    ...
    queryKey: ["stockEntryList", page, from_date, to_date],
    queryFn: async () => {
      const addOneDay = to_date
        ? moment(to_date).add(1, "days").format("y-MM-DD")
        : "";

      const res = await stockApi.adminStockEntryList(
        parseInt(page),
        from_date,
        addOneDay
      );
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            color="primary"
            className="!px-7 !py-3 !text-sm !capitalize"
            onClick={() => downloadExcel("entryList", "Entry List")}
            startIcon={<Download />}
          ...
          {data?.data?.length <= 0 && (
            <div className="!mt-5">
              <Alert severity="error">
                <Typography>No Entry List</Typography>
              </Alert>
            </div>
          )}
